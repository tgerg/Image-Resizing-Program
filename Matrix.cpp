...
int* Matrix_at(Matrix* mat, int row, int column) {
  ...
  int * ptr = mat->data + column + (Matrix_width(mat) * row);
  return ptr;
}
...
const int* Matrix_at(const Matrix* mat, int row, int column) {
  ...
  return ptr + column + (Matrix_width(mat) * row);
}
...
int Matrix_column_of_min_value_in_row(const Matrix* mat, int row,
  ...
  assert(0 <= row && row < Matrix_height(mat));
  int w = Matrix_width(mat);
  assert(0 <= column_start && column_start < column_end && column_end <= w);
  int min = *Matrix_at(mat, row, column_start);
  int min_c = column_start;
  for (int c = column_start + 1; c < column_end; ++c){
    int num = *Matrix_at(mat, row, c);
    if (min > num){
      min = num;
      min_c = c;
    }
